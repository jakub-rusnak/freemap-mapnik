...
function layers(shading, contours, hikingTrails, bicycleTrails, skiTrails, horseTrails) {
  return map => map
    ...
    .sqlLayer('water_line',
      "select geometry, type, tunnel, CASE WHEN interminnent OR seasonal THEN '6,3' ELSE '1000,0' END AS dasharray from osm_waterways_gen1",
      { maxZoom: 11 },
    )
    .sqlLayer('water_line',
      "select geometry, type, tunnel, CASE WHEN interminnent OR seasonal THEN '6,3' ELSE '1000,0' END AS dasharray from osm_waterways",
      { minZoom: 12 },
    )
    .sqlLayer('trees',
